...
def main(args):
    ...
    missing_files = [p for p in checkpoint_paths if p and not os.path.exists(p)]
    if missing_files:
        print(f"Warning: Required checkpoint files missing: {missing_files}")
        print("Skipping SadTalker processing (will use fallback)")
        # Create empty result to signal success to calling process
        os.makedirs(args.result_dir, exist_ok=True)
        with open(os.path.join(args.result_dir, "PROCESSED_WITH_FALLBACK"), 'w') as f:
            f.write("This directory was created by SadTalker fallback mode")
    ...
    try:
        # Handle both old and new generate method return values
        result = preprocess_model.generate(pic_path, first_frame_dir, args.preprocess,\
